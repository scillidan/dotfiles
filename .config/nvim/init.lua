...
if vim.fn.has("win32") == 1 then
	...
	vim.g.clipboard = {
		...
		paste = {
			["+"] = has_pwsh
					and "pwsh.exe -NoLogo -NoProfile -c [Console]::Out.Write($(Get-Clipboard -Raw).ToString())"
				or "powershell.exe -NoLogo -NoProfile -c [Console]::Out.Write($(Get-Clipboard -Raw).ToString())",
			["*"] = has_pwsh
					and "pwsh.exe -NoLogo -NoProfile -c [Console]::Out.Write($(Get-Clipboard -Raw).ToString())"
				or "powershell.exe -NoLogo -NoProfile -c [Console]::Out.Write($(Get-Clipboard -Raw).ToString())",
		},
