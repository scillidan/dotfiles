...
if rl.getbinding then
	...
	if
		...
		or tab == "menu-complete-backward"
		or tab == "old-menu-complete"
		or tab == "old-menu-complete-backward"
		or tab == "clink-select-complete"
		or tab == "clink-popup-complete"
	then
		tab_binding = tab
	end
end
...
add_help_desc("luafunc:fzf_menucomplete", "Use fzf for completion after ** otherwise use 'menu-complete' command")
...
add_help_desc("luafunc:fzf_tab", "Use fzf for completion if ** is immediately before the cursor position")
...
local function create_argmatcher()
	...
	addexflags(argmatcher, {
		...
		{
			"-i",
			"Case-insensitive match (default: smart-case match; +i for case-sensitive match)",
		},
		...
		{
			"--scrollbar=" .. scrollbarchars,
			"C1[C2]",
			"Scrollbar character for main [and preview] window",
			opteq = false,
		},
